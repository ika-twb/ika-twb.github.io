...
- `volume-name`: The mapped name of the decrypted volume, appearing in
  `/dev/mapper/`.
- `encrypted-device`: The underlying encrypted partition (e.g.,
  `/dev/sdb1` or a UUID path).
- `key-file` Can be `none` (manual entry), a key file path, or `tmp` for
  a random key.
- `options`: Parameters passed to `cryptsetup`, such as `luks`,
  `discard`, or `timeout`.
...
1.  Automatic Unlock with a Key File
    ...
    `/etc/crypttab` to unlock the partition at boot:
...
2.  Encrypted Swap to Prevent Data Leakage
    ...
    Linux swap partitions can leak sensitive data. Encrypt swap
    dynamically with a random key on each boot:
...
3.  Remote Unlocking via SSH
    ...
    If a server reboots and you need to unlock its encrypted disk
    remotely:
...
4.  Skipping Password Prompt During Boot with Initramfs
    ...
    By default, LUKS-encrypted root partitions require a password at
    boot. To bypass this:
    ...
    - Now the encrypted partition unlocks without user input during
      boot.

# Why Use It?

- Automates unlocking encrypted partitions, reducing downtime and manual
  intervention.
- Essential for headless servers, remote systems, and automated
  deployments.
- Prevents boot delays while maintaining security.
- Works seamlessly with `systemd` to ensure encrypted volumes are ready
  before mounting.
